...
class TestDownload(unittest.TestCase):
    def test_download_uses_timeout(self):
        downloader = getattr(audit_snapshot, 'download_file', None)
        ...
        with tempfile.TemporaryDirectory() as td, mock.patch.object(
            ...
            destination = Path(td) / "source.tar.gz"
            downloader(
        ...
        urlopen.assert_called_once_with(
            "https://example.invalid/source.tar.gz", timeout=17
        )
...
''')
    ...
    def test_check_rejects_conflicting_source_identity(self):
        fixture_dir, report_path = self._write_archive_and_report()
        self.addCleanup(fixture_dir.cleanup)
        ...
        with mock.patch.object(
            ...
            audit_snapshot.main()
        self.assertIn('--url conflicts with report identity', stderr.getvalue())

    def test_check_accepts_matching_explicit_identity(self):
        fixture_dir, report_path = self._write_archive_and_report()
        self.addCleanup(fixture_dir.cleanup)
        identity = json.loads(report_path.read_text())['upstream']
        argv = ['audit_snapshot.py', '--check', str(report_path)]
        for option in ('tag', 'commit', 'url', 'sha256'):
            argv.extend((f'--{option}', identity[option]))
