// compat.opencvwin (windows-x86_64 CORE) end-to-end: the mcpp clang-MSVC source
// build exposes core/imgproc/imgcodecs — Mat ops, imgproc, PNG codec roundtrip —
// with x86_64 SIMD dispatch, headless (no Win32 GUI backend). windows-only.
#ifdef _WIN32
...
int main() {
    ...
    std::printf("compat.opencvwin 5.0.0 ok: core/imgproc/imgcodecs (x86_64 SIMD, headless)\n");
    return 0;
}
#else
int main() { return 0; }
