-- compat.curand — the CUDA cuRAND, as an ordinary mcpp dependency.
--
-- cuRAND: random number generation on the device. It is a redistributable component, so `xim:libcurand` carries the
...
package = {
    ...
    mcpp = {
        ...
        import_std   = false,
        c_standard   = "c11",
        sources      = { "mcpp_generated/curand_anchor.c" },
        targets      = { ["curand"] = { kind = "lib" } },
        include_dirs = { "mcpp_generated/curand/include" },
        deps         = { ["compat.cudart"] = "12.9.79" },
        linux = { ldflags = { "-Lmcpp_generated/curand/lib", "-lcurand" } },
        runtime = {
            library_dirs = { "mcpp_generated/curand/lib" },
            capabilities = { "cuda.curand" },
            provides     = { "cuda.curand" },
        },
    },
}
...
function install()
    ...
    os.mkdir(generated)
    io.writefile(path.join(generated, "curand_anchor.c"),
        "int mcpp_compat_curand_anchor(void) { return 0; }\n")

    local src = payload_dir("xim:libcurand", "libcurand", "10.3.10.19")
    if not src then
        error("compat.curand: xim:libcurand@10.3.10.19 was not found; the payload edge in "
              .. "xpm.linux.deps should have installed it")
    end

    local root = path.join(generated, "curand")
    ...
    -- link line, not a farm entry that `-lcurand` would silently prefer.
    ...
    if nh == 0 or nl == 0 then
        error(string.format("compat.curand: the payload at %s carries %d headers and %d "
                            .. "shared libraries; both must be non-zero", src, nh, nl))
    end
    log.info("compat.curand: %d headers, %d libraries, %d C-library stubs from %s",
             nh, nl, ns, src)
    return true
end
