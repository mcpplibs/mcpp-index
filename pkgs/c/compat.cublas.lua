-- compat.cublas — the CUDA cuBLAS, as an ordinary mcpp dependency.
--
-- cuBLAS: dense linear algebra on the device. It is a redistributable component, so `xim:libcublas` carries the
...
--
-- `libnvblas.so` is farmed with the rest but not named on the link line: it is a
-- host BLAS interposer, a different contract from the cuBLAS API this package
-- exposes, and a project that wants it says so itself.
--
...
package = {
    ...
    mcpp = {
        ...
        import_std   = false,
        c_standard   = "c11",
        sources      = { "mcpp_generated/cublas_anchor.c" },
        targets      = { ["cublas"] = { kind = "lib" } },
        include_dirs = { "mcpp_generated/cublas/include" },
        deps         = { ["compat.cudart"] = "12.9.79" },
        linux = { ldflags = { "-Lmcpp_generated/cublas/lib", "-lcublas", "-lcublasLt" } },
        runtime = {
            library_dirs = { "mcpp_generated/cublas/lib" },
            capabilities = { "cuda.cublas" },
            provides     = { "cuda.cublas" },
        },
    },
}
...
function install()
    ...
    os.mkdir(generated)
    io.writefile(path.join(generated, "cublas_anchor.c"),
        "int mcpp_compat_cublas_anchor(void) { return 0; }\n")

    local src = payload_dir("xim:libcublas", "libcublas", "12.9.1.4")
    if not src then
        error("compat.cublas: xim:libcublas@12.9.1.4 was not found; the payload edge in "
              .. "xpm.linux.deps should have installed it")
    end

    local root = path.join(generated, "cublas")
    ...
    -- link line, not a farm entry that `-lcublas` would silently prefer.
    ...
    if nh == 0 or nl == 0 then
        error(string.format("compat.cublas: the payload at %s carries %d headers and %d "
                            .. "shared libraries; both must be non-zero", src, nh, nl))
    end
    log.info("compat.cublas: %d headers, %d libraries, %d C-library stubs from %s",
             nh, nl, ns, src)
    return true
end
