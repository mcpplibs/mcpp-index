-- compat.cufft — the CUDA cuFFT, as an ordinary mcpp dependency.
--
-- cuFFT: fast Fourier transforms on the device. It is a redistributable component, so `xim:libcufft` carries the
...
--
-- `libcufftw` is the FFTW-compatible interface and is named alongside `cufft`;
-- the two are one component upstream and a consumer picking either finds it.
--
...
package = {
    ...
    mcpp = {
        ...
        import_std   = false,
        c_standard   = "c11",
        sources      = { "mcpp_generated/cufft_anchor.c" },
        targets      = { ["cufft"] = { kind = "lib" } },
        include_dirs = { "mcpp_generated/cufft/include" },
        deps         = { ["compat.cudart"] = "12.9.79" },
        linux = { ldflags = { "-Lmcpp_generated/cufft/lib", "-lcufft", "-lcufftw" } },
        runtime = {
            library_dirs = { "mcpp_generated/cufft/lib" },
            capabilities = { "cuda.cufft" },
            provides     = { "cuda.cufft" },
        },
    },
}
...
function install()
    ...
    os.mkdir(generated)
    io.writefile(path.join(generated, "cufft_anchor.c"),
        "int mcpp_compat_cufft_anchor(void) { return 0; }\n")

    local src = payload_dir("xim:libcufft", "libcufft", "11.4.1.4")
    if not src then
        error("compat.cufft: xim:libcufft@11.4.1.4 was not found; the payload edge in "
              .. "xpm.linux.deps should have installed it")
    end

    local root = path.join(generated, "cufft")
    ...
    -- link line, not a farm entry that `-lcufft` would silently prefer.
    ...
    if nh == 0 or nl == 0 then
        error(string.format("compat.cufft: the payload at %s carries %d headers and %d "
                            .. "shared libraries; both must be non-zero", src, nh, nl))
    end
    log.info("compat.cufft: %d headers, %d libraries, %d C-library stubs from %s",
             nh, nl, ns, src)
    return true
end
