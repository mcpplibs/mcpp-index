-- compat.cusolver — the CUDA cuSOLVER, as an ordinary mcpp dependency.
--
-- cuSOLVER: dense and sparse direct solvers on the device. It is a redistributable component, so `xim:libcusolver` carries the
...
--
-- `libcusolverMg` is the multi-GPU interface, one component upstream and named
-- alongside the single-GPU one for the same reason.
--
...
package = {
    ...
    xpm = {
        linux = {
            deps = { "xim:libcusolver@11.7.5.82" },
            ["11.7.5.82"] = {
    ...
    mcpp = {
        ...
        import_std   = false,
        c_standard   = "c11",
        sources      = { "mcpp_generated/cusolver_anchor.c" },
        targets      = { ["cusolver"] = { kind = "lib" } },
        include_dirs = { "mcpp_generated/cusolver/include" },
        deps         = { ["compat.cudart"] = "12.9.79" },
        linux = { ldflags = { "-Lmcpp_generated/cusolver/lib", "-lcusolver", "-lcusolverMg" } },
        runtime = {
            library_dirs = { "mcpp_generated/cusolver/lib" },
            capabilities = { "cuda.cusolver" },
            provides     = { "cuda.cusolver" },
        },
    },
}
...
function install()
    ...
    os.mkdir(generated)
    io.writefile(path.join(generated, "cusolver_anchor.c"),
        "int mcpp_compat_cusolver_anchor(void) { return 0; }\n")

    local src = payload_dir("xim:libcusolver", "libcusolver", "11.7.5.82")
    if not src then
        error("compat.cusolver: xim:libcusolver@11.7.5.82 was not found; the payload edge in "
              .. "xpm.linux.deps should have installed it")
    end

    local root = path.join(generated, "cusolver")
    ...
    -- link line, not a farm entry that `-lcusolver` would silently prefer.
    ...
    if nh == 0 or nl == 0 then
        error(string.format("compat.cusolver: the payload at %s carries %d headers and %d "
                            .. "shared libraries; both must be non-zero", src, nh, nl))
    end
    log.info("compat.cusolver: %d headers, %d libraries, %d C-library stubs from %s",
             nh, nl, ns, src)
    return true
end
