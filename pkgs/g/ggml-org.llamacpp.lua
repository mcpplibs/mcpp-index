-- ggml-org.llamacpp — llama.cpp b10069 registry + llama implementation
--
-- 消费者引入:
--     mcpp add ggml-org.llamacpp@b10069
--
-- 拥有 ggml-backend register/dynamic-loader 和 llama 推理实现 + 模型 TU.
-- 默认或 backend-cpu 使用 CPU；backend-metal 仅支持 macOS AArch64.
-- 其他 feature 由 build.mcpp 在编译前拒绝.
...
int main() try {
    ...
    return 1;
        ...
        flags = {
            ...
            { glob = "*/src/models/embed.cpp",           cxxflags = { "-std=c++20" } },
            ...
            { glob = "*/src/models/minimax-text-01.cpp", cxxflags = { "-std=c++20" } },
        },
        linux = {
            sources = {
